...
| プロパティ名            | 型                    | 説明                 |
| ----------------------- | --------------------- | -------------------- |
| SQLALCHEMY_DATABASE_URI | sqlalchemy.engine.URL | データベース接続 URI |
...
#### デコレータ
- `@computed_field`{lang=python}： Pydantic の計算フィールドを定義するデコレータ。
- `@cached_property`{lang=python}： プロパティの結果をキャッシュするデコレータ。
...
正規表現 `r".*\{repository_id\}.*"`{lang=python} にマッチする文字列である。
...
正規表現 `r".*\{repository_id\}.*\{user_defined_id\}.*"`{lang=python} にマッチする文字列である。
