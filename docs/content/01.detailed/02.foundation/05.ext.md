...
| 名前    | 型                                                 | 説明                                                                                 |
| ------- | -------------------------------------------------- | ------------------------------------------------------------------------------------ |
| _config | str \| [RuntimeConfig](02.config.md#runtimeconfig) | （プライベート）サーバー設定のファイルパス、またはサーバー設定インスタンスを保持する |
...
3. [`logger:setup_logger`](08.logger.md#setup_logger) 関数を呼び出し、アプリケーションのロガーの設定を行う。
4. API のルーティングを登録する。
