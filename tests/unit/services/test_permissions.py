...
mock_affiliations = Affiliations(
    roles=[
        _RoleGroup(repository_id="repo1", roles=[USER_ROLES.REPOSITORY_ADMIN]),
        _RoleGroup(repository_id="repo2", roles=[USER_ROLES.REPOSITORY_ADMIN]),
        _RoleGroup(repository_id="repo3", roles=[USER_ROLES.CONTRIBUTOR]),
    ],
    groups=[
...
def test_get_login_user_roles(app, mocker: MockerFixture):
    mocker.patch(
        "server.services.permissions.extract_group_ids",
        return_value=["group1", "jc_roles_sysadm_test"],
    )
    ...
    with app.test_request_context("/"):
        login_user(test_sys_admin_user)
        affiliations = permissions.get_login_user_roles()
